...
import com.sahara.dictionary.util.MyStringUtils;
import com.sahara.dictionary.util.SignEnum;
...
public class BuildPDF {
    ...
    private static Pattern myPattern = Pattern.compile("\\s*|\r|\n");
...
//            rect.setBackgroundColor(BaseColor.WHITE);
            ...
            for (int i = 0; i < tableInfos.size(); i++) {
                ...
                PdfPTable indexTable = refinePdfChapter_old(indexHeader, tableHeader, doc, tableInfo, i + 1, font);
                doc.add(indexTable);
            }
            doc.close();
        } catch (Exception e) {
        ...
        for (TableInfo info : list) {
            ...
            for (int k = 0; k < columnInfos.size(); k++) {
                ...
                if (null == Column.getDescription()) {
                    ...
                    //str = str.replaceAll("\n","");
                    str = MyStringUtils.delete_R_N(str);
                    if (str.length() == 0) {
                        oneTble.append("...").append("|").append("\n");
                    } else {
...
//                }
    ...
    public static void build(String FILE_DIR, List<TableInfo> tableInfos, String pdfName) throws Exception {
        ...
        String[] indexHeader = {"序列", "索引名", "类型", "包含字段"};
        for (TableInfo tableInfo : tableInfos) {
            Chapter chapter = refinePdfChapterLocal(indexHeader, tableHeader, doc, tableInfo, order, font, cnFont);
            //加入文档中
            doc.add(chapter);
            //保存章节内容
        ...
        //生成本地PDF文件
        makePdfFileLocal(tableInfos, rect, chapterList, event, FILE_DIR, pdfName);
    }
    ...
    public static void getDocumentBuild(List<TableInfo> tableInfos, HttpServletResponse response) throws Exception {
        ...
        for (int i = 0; i < tableInfos.size(); i++) {
            TableInfo tableInfo = tableInfos.get(i);
            Chapter chapter = refinePdfChapter(indexHeader, tableHeader, doc, tableInfo, i + 1, font, cnFont);
            //加入文档中
            doc.add(chapter);
            //保存章节内容
            chapterList.add(chapter);
        ...
        //存目录监听 结束
        stopWatch.stop();
        System.out.println("文件章节内容组装--->执行时间：" + stopWatch.getTotalTimeMillis() + "毫秒");

        //生成PDF文件流 并网络回写给浏览器
        makePdfFile(tableInfos, response, rect, chapterList, event);
    }
     ...
     */
    private static Chapter refinePdfChapter(String[] indexHeader, String[] tableHeader, Document doc, TableInfo tableInfo, int order, Font font, Font cnFont) {
        try {
     ...
     * @param event
     */
    private static void makePdfFile(List<TableInfo> tableInfos, HttpServletResponse response, Rectangle rect, List<Chapter> chapterList, ContentEvent event) {
...
//        FileOutputStream os = new FileOutputStream(FILE_DIR + pdfName + ".pdf");
    ...
    @Deprecated
    private static PdfPTable refinePdfChapter_old(String[] indexHeader, String[] tableHeader, Document doc, TableInfo tableInfo, int order, Font font) throws DocumentException {
        tableInfo = setIsIndex(tableInfo);
        // 生成锚点
     ...
     * @throws DocumentException
     */
    private static Chapter refinePdfChapterLocal(String[] indexHeader, String[] tableHeader, Document doc, TableInfo tableInfo, int order, Font font, Font cnFont) {
        try {
            tableInfo = setIsIndex(tableInfo);
            Chapter chapter = new Chapter(new Paragraph(tableInfo.getTableName()), order);
            //设置跳转地址
     ...
     * @param event
     */
    private static void makePdfFileLocal(List<TableInfo> tableInfos, Rectangle rect, List<Chapter> chapterList, ContentEvent event, String FILE_DIR, String pdfName) throws IOException, DocumentException {
